...
from agent_handoff.agent import AgentLoop
...
def _cmd_help():
    ...
    cmds = [
        ("/agent <task>",   "run agent in ARC mode (strict tool executor)"),
...
# ── Agent CLI display layer (ARC mode) ───────────────────────────────────────

def _agent_separator(label: str = ""):
    """Print a clean section separator."""
    w = min(_term_width() - 4, 90)
    if label:
        line = f"--- {label} "
        pad = w - len(line)
        print(f"\n  {C.BBLACK}{line}{'-' * max(0, pad)}{C.RESET}")
    else:
        print(f"\n  {C.BBLACK}{'-' * w}{C.RESET}")


def _agent_section(title: str, content: str, status_color: str = ""):
    """Print a labeled section block."""
    color = status_color or C.RESET
...
def run_agent_interactive(
    ...
    """Run agent in ARC mode with full CLI transparency.

    Renders structured ASCII blocks: PLAN, STEP, TOOL CALL, RESULT,
    VERIFY, FINAL SUMMARY. No emojis, no decorative text.
    """
    ...
    agent = AgentLoop(model=model, client=client, max_steps=8, cwd=cwd)

    # --- PLAN phase ---
    spinner = Spinner("Planning...", color=C.CYAN).start()
    plan_steps = agent.plan(task)
    spinner.stop()

    _agent_separator("PLAN")
    ...
    def on_step(step_num: int, event: str, data: dict):
        nonlocal current_step_num

        if event == "model_call":
            current_step_num = step_num
            _agent_separator(f"STEP {step_num}/8 | MODEL: {data['model']}")

        elif event == "tool_call":
            category = data.get("category", "")
            action = data.get("action", "")
            params = data.get("params", {})
            # Format params without content (too long)
            param_strs = []
            for k, v in params.items():
                ...
                if k == "content":
                    ...
                    sv = sv[:57] + "..."
                param_strs.append(f'{k}="{sv}"')
            param_display = " ".join(param_strs)
            print(f"\n  TOOL CALL:")
            print(f"  {category} {action} {param_display}")
            ...
            if success:
                ...
                print(f"  {C.RED}FAIL: {error}{C.RESET}")

        elif event == "invalid":
            retry = data.get("retry", 0)
            max_retries = data.get("max_retries", 2)
            raw = data.get("raw", "")[:80]
            print(f"\n  REJECTED: invalid output (retry {retry}/{max_retries})")
            print(f"  Raw: {raw}")
            ...
            print(f"\n  {C.RED}ABORT: {reason}{C.RESET}")

    result = agent.run(task, on_step=on_step)
    elapsed = _time.perf_counter() - t0

    # --- FINAL SUMMARY ---
    _agent_separator("FINAL SUMMARY")
    if result.files_created:
        print(f"  Files created:  {', '.join(result.files_created)}")
    if result.files_modified:
        print(f"  Files modified: {', '.join(result.files_modified)}")
    if result.files_deleted:
        print(f"  Files deleted:  {', '.join(result.files_deleted)}")
    if result.commands_run:
        print(f"  Commands run:   {', '.join(result.commands_run)}")
    if not any([result.files_created, result.files_modified, result.files_deleted, result.commands_run]):
    ...
    return result.message if result.status == "SUCCESS" else None
...
def repl(
    ...
    workspace_dir: Optional[str] = None  # workspace root for ARC tools (lazy init)
    loaded_files: Dict[str, str] = {}   # {filename: content} — files attached to context

    def _prompt_for_workspace_dir() -> str:
        """Prompt user to select (or create) a workspace directory."""
        while True:
            raw_choice = input(
                "No workspace set.\n\n"
                "Select an option:\n"
            ...
            if raw_choice == "1":
                ...
                while True:
                    ...
                    if not name:
                        print(f"  {C.RED}✗{C.RESET} Workspace name cannot be empty.")
                        continue
                    # Treat name as a simple directory name, not a path.
                    if "/" in name or "\\" in name or os.sep in name:
                        print(f"  {C.RED}✗{C.RESET} Use a simple name (no path separators).")
                        continue
                    base = Path(os.getcwd())
                    candidate = base / name
                    if candidate.exists():
                        print(f"  {C.RED}✗{C.RESET} Directory already exists. Choose another name.")
                        continue
                    candidate.mkdir(parents=False, exist_ok=False)
                    return str(candidate.resolve())
            elif raw_choice == "3":
                while True:
                    raw_path = input("Enter path: ").strip()
                    if not raw_path:
                        print(f"  {C.RED}✗{C.RESET} Path cannot be empty.")
                        continue
                    ...
                    if p.exists() and p.is_dir():
                        return str(p)
                    print(f"  {C.RED}✗{C.RESET} Invalid directory path. Please try again.")
            else:
                print(f"  {C.RED}✗{C.RESET} Invalid choice. Enter 1, 2, or 3.")
    ...
    while True:
        try:
            ws_label = f" {C.DIM}[workspace: {Path(workspace_dir).name}]{C.RESET}" if workspace_dir else ""
            prompt_str = f"  {C.BMAGENTA}{C.BOLD}❯{C.RESET}{ws_label} "
        ...
        if query.startswith("/"):
            ...
            if cmd in ("/exit", "/quit", "/q"):
                ...
                if not q:
                    print(f"  {C.RED}Usage: /agent <task>{C.RESET}\n")
                else:
                    try:
                        print()
                        # Lazy workspace initialization: never run ARC tools without a workspace root.
                        if workspace_dir is None:
                            workspace_dir = _prompt_for_workspace_dir()
                            print(f"Workspace set to: {workspace_dir}")

                        agent_result = run_agent_interactive(
                            client=client,
                            model=model_small,
                            task=q,
                            cwd=workspace_dir,
                ...
                action = sub_parts[0].lower() if sub_parts else "show"
                ...
                if action in ("show", "?"):
                    ...
                    print("  Usage: /workspace set [path] | /workspace show | /workspace reset\n")
